    ...
    <div class="panel selected" data-panel="setup">
      <div class="card">
        ...
        <div class="grid">
          ...
          <label>Model inactivity seconds<input id="cfg-inactivity" type="number" min="30" max="86400"></label>
        </div>
        <p class="note">Stops a model request only when no streamed data arrives for this long. Maximum minutes remains the total job limit.</p>
        <div class="checks">
...
$('#cfg-name').value=c.name;$('#cfg-runtime').value=c.max_runtime_minutes;$('#cfg-cycles').value=c.max_cycles;$('#cfg-temp').value=c.temperature_limit_c??'';$('#cfg-tokens').value=c.max_tokens;$('#cfg-rounds').value=c.max_rounds;$('#cfg-inactivity').value=c.request_inactivity_timeout_seconds??c.request_timeout_seconds??3600;$('#cfg-approve').checked=c.auto_approve;$('#cfg-stop').checked=c.stop_on_failure;
